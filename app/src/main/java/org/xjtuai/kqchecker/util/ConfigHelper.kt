...
object ConfigHelper {
    ...
    private fun readConfig(context: Context): Config {
        ...
        return Config(
            ...
            authRedirectPrefix = resolveString(
                "auth_redirect_prefix",
                defaults.authRedirectPrefix,
                privateJson,
                publicJson
            ),
            weeklyEndpoint = normalizeEndpoint(
                resolveString("weekly_endpoint", defaults.weeklyEndpoint, privateJson, publicJson)
            ),
            waterListEndpoint = normalizeEndpoint(
                resolveString("water_list_endpoint", defaults.waterListEndpoint, privateJson, publicJson)
            ),
            currentTermEndpoint = normalizeEndpoint(
                resolveString(
                    "current_term_endpoint",
                    defaults.currentTermEndpoint,
                    privateJson,
                    publicJson
                )
            ),
            competitionBaseUrl = resolveString(
                "competition_base_url",
                defaults.competitionBaseUrl,
                privateJson,
                publicJson
            ),
            competitionEndpoint = normalizeEndpoint(
                resolveString("competition_endpoint", defaults.competitionEndpoint, privateJson, publicJson)
            ),
            competitionApiKey = resolveString(
                "competition_api_key",
                defaults.competitionApiKey,
                privateJson,
                publicJson
            ),
