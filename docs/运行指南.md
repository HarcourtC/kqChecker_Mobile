...
4. **查看应用日志**
   ...
   - 特别关注是否有正确读取`termNo=1`和`week=1`的日志记录
...
8. **竞赛数据功能测试**（2025-02 新增）
   ...
   - 强制清除应用缓存文件（在需要让应用重新从网络拉取时使用）：
      ...
      adb shell run-as org.xjtuai.kqchecker rm -f /data/data/org.xjtuai.kqchecker/files/weekly_raw.json;
      adb shell run-as org.xjtuai.kqchecker rm -f /data/data/org.xjtuai.kqchecker/files/weekly_cleaned.json
      ```
   ...
   - 查看/拉取缓存文件（可用 `run-as` 权限）：
      ...
      adb shell run-as org.xjtuai.kqchecker cat /data/data/org.xjtuai.kqchecker/files/weekly_raw.json > weekly_raw.json
      adb pull /data/data/org.xjtuai.kqchecker/files/weekly_cleaned.json .
      ```
